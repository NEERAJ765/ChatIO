...
const RightSidebar = () => {
  ...
  // get all images for the messages to the state
  ...
  return selectedUser &&  (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll 
        ...
        <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto'>
          <img src={selectedUser?. profilePic || assets.avatar_icon} alt="avatar_icon not found"
              ...
              <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
                  {msgImages.map((url,index) => (
                    <div key={index} onClick={() => window.open(url)} className='cursor-pointer rounded'>
                          <img src={url} alt="image url not found" className='h-full rounded-md' />
                    </div>
                  ))}
              </div>
        </div>
